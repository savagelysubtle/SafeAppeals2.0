...
- Add imports for getPathForFile and DataTransfers to SidebarChat.tsx
- Add logic to handle DataTransfers.RESOURCES for VSCode Explorer drags
- Replace (file as any).path with getPathForFile() utility
- Implement collectFilesFromFolder helper and integrate into handleDrop
- Handle text/uri-list format as fallback for edge cases
- Add file size checks (1MB warning, 50MB max)
- Add confirmation dialogs for 10+ files
- Basic file filtering (node_modules, .git, dist, build, etc.)
- VSCode notifications for success/errors/warnings
- Update drag overlay to show folder-specific messages and file counts
...
1. **Pattern Matching**: VSCode uses glob patterns, need proper library (minimatch or VSCode's internal)
2. **Multiple Exclusion Sources**:
