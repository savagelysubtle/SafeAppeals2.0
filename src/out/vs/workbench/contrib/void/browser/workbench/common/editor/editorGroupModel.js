...
export class EditorGroupModel extends Disposable {
    ...
    static IDS = 0;
    //#region events
    _onDidModelChange = this._register(new Emitter({ leakWarningThreshold: 500 /* increased for users with hundreds of inputs opened */ }));
    ...
    locked = false;
    selection = []; // editors in selected state, first one is active
    get active() {
        return this.selection[0] ?? null;
    }
    preview = null; // editor in preview state
    sticky = -1; // index of first editor in sticky state
    transient = new Set(); // editors in transient state
