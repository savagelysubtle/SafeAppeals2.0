...
export class VoidCommandBarService extends Disposable {
    ...
    stateOfURI = {};
    sortedURIs = []; // keys of state (depends on diffZones in the uri)
    _listenToTheseURIs = new Set(); // uriFsPaths
    // Emits when a URI's stream state changes between idle, streaming, and acceptRejectAll
    _onDidChangeState = new Emitter();
    onDidChangeState = this._onDidChangeState.event;
    // active URI
    activeURI = null;
    _onDidChangeActiveURI = new Emitter();
    onDidChangeActiveURI = this._onDidChangeActiveURI.event;
