...
export const FileOrganizerDashboard: React.FC = () => {
  ...
  if (viewMode === 'docket') {
    return (
      <div className="void-void-scope" style={{ height: '100%', width: '100%' }}>
				<DocketDashboard />
			</div>);
  ...
  const fileOrganizerService = useMemo(() => {
    try {
      ...
      console.error(
        "[FileOrganizerDashboard] Failed to get FileOrganizerService:",
        error
      );
      return null;
    }
    ...
    const loadInfo = async () => {
      if (fileOrganizerService) {
        try {
          ...
          if (workspaceFolder) {
            const info = await fileOrganizerService.loadCaseInfo(
              workspaceFolder
            );
            if (info) {
              setCaseInfo(info);
          ...
          console.error(
            "[FileOrganizerDashboard] Error loading case info:",
            error
          );
        }
      }
  ...
  const handleManualRename = useCallback((fileUri: string, newName: string) => {
    console.log("[Dashboard] Setting manual rename:", fileUri, newName);
        ...
        style={{
          ...
          style={{
            ...
            color: "var(--vscode-descriptionForeground)"
					...
					💡 Tip: Set up your case info in the <strong>Case Info</strong>{" "}
					sidebar panel first
				</p>
			</div>
