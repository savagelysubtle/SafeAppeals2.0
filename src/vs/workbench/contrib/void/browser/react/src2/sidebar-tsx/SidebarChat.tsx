...
import {
  ChatMode,
  FeatureName,
  isFeatureNameDisabled } from
"../../../../../../../workbench/contrib/void/common/voidSettingsTypes.js";
import {
  ChatMessage,
  CheckpointEntry,
  StagingSelectionItem,
  ToolMessage } from
"../../../../common/chatThreadServiceTypes.js";
import { removeMCPToolNamePrefix } from "../../../../common/mcpServiceTypes.js";
import {
  isABuiltinToolName,
  MAX_FILE_CHARS_PAGE } from
"../../../../common/prompt/prompts.js";
...
import {
  ...
  LintErrorItem,
  ToolName } from
"../../../../common/tools/toolsServiceTypes.js";
...
import {
  ...
  IconShell1,
  StatusIndicator,
  useEditToolStreamState } from
"../markdown/ApplyBlockHoverButtons.js";
import {
  ChatMarkdownRender,
  ChatMessageLocation,
  getApplyBoxId } from
"../markdown/ChatMarkdownRender.js";
import {
  BlockCode,
  TextAreaFns,
...
import {
  ...
  ToolHeaderWrapper,
  toolNameToDesc,
  type ToolHeaderParams } from
"./tool-renderers/index.js";
...
}: {message: ChatMessage;messageIdx: number;onCopy: () => void;onEdit?: () => void;onRegenerate?: () => void;onDelete?: () => void;showEdit?: boolean;showRegenerate?: boolean;showDelete?: boolean;}) => {
  return (
    <div className="void-flex void-gap-1 void-opacity-0 group-hover:void-opacity-100 void-transition-opacity void-absolute void-right-2 void-top-2 void-bg-void-bg-2 void-rounded void-p-1 void-border void-border-void-border-2">
			<button
        onClick={onCopy}
        className="void-p-1 hover:void-bg-void-bg-1 void-rounded void-transition-colors"
        data-tooltip-id="void-tooltip"
        data-tooltip-content="Copy message">
        
				<CopyIcon size={12} className="void-text-void-fg-3" />
			</button>
			{showEdit && onEdit &&
      <button
        onClick={onEdit}
        ...
        data-tooltip-content="Edit message">
					...
					<Pencil size={12} className="void-text-void-fg-3" />
				</button>
      }
      ...
      <button
        ...
        data-tooltip-content="Regenerate response">
					...
					<RotateCw size={12} className="void-text-void-fg-3" />
				</button>
      }
			{showDelete && onDelete &&
      <button
        ...
        data-tooltip-content="Delete message">
					...
					<Trash2 size={12} className="void-text-red-400" />
				</button>
      }
		</div>);
...
export const VoidChatArea: React.FC<VoidChatAreaProps> = ({
  ...
  return (
    <div
      ref={divRef}
      className={` void-gap-x-1 void-flex void-flex-col void-p-2 void-relative void-input void-text-left void-shrink-0 void-rounded-md void-bg-void-bg-1 void-transition-all void-duration-200 void-border ${
      ...
      isDragging ? "void-border-void-button-primary void-border-2 void-bg-void-button-primary/10" : "void-border-void-border-3 focus-within:void-border-void-border-1 hover:void-border-void-border-1"} void-max-h-[80vh] void-overflow-y-auto void-void-scrollbar ${
      ...
      onDragLeave={handleDragLeave}>
			...
			{isDragging &&
      <div className="void-absolute void-inset-0 void-flex void-items-center void-justify-center void-bg-void-button-primary/20 void-z-50 void-pointer-events-none void-rounded-md">
					<div className="void-text-void-button-primary void-font-medium void-flex void-items-center void-gap-2">
      ...
      <SelectedFiles
        ...
        <div className="void-flex void-flex-col void-gap-y-1">
						<div className="void-flex void-items-center void-flex-wrap void-gap-x-2 void-gap-y-1 void-text-nowrap ">
							{featureName === "Chat" &&
            <ChatModeDropdown className="void-text-xs void-text-void-fg-3 void-bg-void-bg-1 void-border void-border-void-border-2 void-rounded void-py-0.5 void-px-1" />
            }
							<ModelDropdown
              featureName={featureName}
              className="void-text-xs void-text-void-fg-3 void-bg-void-bg-1 void-rounded" />
...
}: {children: React.ReactNode;className?: string;style?: React.CSSProperties;scrollContainerRef: React.MutableRefObject<HTMLDivElement | null>;}) => {
              ...
              data-tooltip-delay-show={3000}>
                ...
                onClick={() => {
                  ...
                  if (isThisSelectionProspective) {
                    ...
                    // TODO!!! reveal in tree
                  } else if (selection.type === "Image") {// Open the image file
                    voidOpenFileFn(selection.uri, accessor);
                  }
                }}>
                ...
                <span
                  className={`void-text-[8px] void-'void-opacity-60 void-text-void-fg-4`}>
                ...
                onClick={(e) => {
                  ...
                  ...selections.slice(i + 1)]
										...
										<IconX className="void-stroke-[2]" size={10} />
									</div> :
...
const EditTool = ({
  ...
  toolMessage.type === "tool_request")
  {
    ...
    <ToolChildrenWrapper className="void-bg-void-bg-3">
  ...
  toolMessage.type === "tool_error")
  {
    ...
    if (toolMessage.type === "success" || toolMessage.type === "rejected") {
      ...
      <BottomChildren title="Lint errors">
        ...
        <div key={i} className="void-whitespace-nowrap">
...
const UserMessageComponent = ({
...
}: {chatMessage: ChatMessage & {role: "user";};messageIdx: number;currCheckpointIdx: number | undefined;isCheckpointGhost: boolean;_scrollToBottom: (() => void) | null;}) => {
  const accessor = useAccessor();
  ...
  return (
    ...
    className={` void-relative void-ml-auto void-group ${
...
const AssistantMessageComponent = ({
...
}: {chatMessage: ChatMessage & {role: "assistant";};isCheckpointGhost: boolean;messageIdx: number;isCommitted: boolean;}) => {
  const accessor = useAccessor();
  const chatThreadsService = accessor.get("IChatThreadService");
  ...
  return (
    <div className="void-relative void-group">
			<MessageActions
        message={chatMessage}
        messageIdx={messageIdx}
        onCopy={handleCopyMessage}
...
}: {isDoneReasoning: boolean;isStreaming: boolean;children: React.ReactNode;}) => {
...
const ToolRequestAcceptRejectButtons = ({
  ...
  <div key={approvalType} className="void-flex void-items-center void-ml-2 void-gap-x-1">
  ...
  return (
    <div className="void-flex void-gap-2 void-mx-0.5 void-items-center">
			{approveButton}
			{cancelButton}
			{approvalToggle}
		</div>);
...
}: {uri: URI | undefined;code: string;type: "diff" | "rewrite";}) => {
  const content =
  type === "diff" ?
...
const LintErrorChildren = ({ lintErrors }: {lintErrors: LintErrorItem[];}) => {
  return (
    <div className="void-text-xs void-text-void-fg-4 void-opacity-80 void-border-l-2 void-border-void-warning void-px-2 void-py-0.5 void-flex void-flex-col void-gap-0.5 void-overflow-x-auto void-whitespace-nowrap">
...
}: {threadId: string;applyBoxId: string;uri: URI;codeStr: string;toolName: "edit_file" | "rewrite_file";}) => {
...
const InvalidTool = ({
...
}: {toolName: ToolName;message: string;mcpServerName: string | undefined;}) => {
  const accessor = useAccessor();
  ...
  componentParams.children =
  <ToolChildrenWrapper>
			<CodeChildren className="void-bg-void-bg-3">{message}</CodeChildren>
		</ToolChildrenWrapper>;
...
}: {toolName: ToolName;mcpServerName: string | undefined;}) => {
  const accessor = useAccessor();
...
const CommandTool = ({
  ...
  if (toolMessage.type === "success") {
    ...
    componentParams.children =
    <ToolChildrenWrapper className="void-whitespace-pre void-text-nowrap void-overflow-auto void-text-sm">
				<div className="!void-select-text void-cursor-auto">
    ...
    if (type === "run_command")
    componentParams.children =
    <div ref={divRef} className="void-relative void-h-[300px] void-text-sm" />;
...
const builtinToolNameToComponent: { [T in
BuiltinToolName]: {resultWrapper: ResultWrapper<T>;} } =
{
  read_file: {
    resultWrapper: ({ toolMessage }) => {
      ...
      if (toolMessage.type === "success") {
        ...
        if (result.hasNextPage && params.pageNumber === 1)
          ...
          Math.round(MAX_FILE_CHARS_PAGE) / 1000}k)`;else
  ...
  read_lint_errors: {
    resultWrapper: ({ toolMessage }) => {
      ...
      if (toolMessage.type === "success") {
        ...
        componentParams.children =
        <LintErrorChildren lintErrors={result.lintErrors} />;else
...
const Checkpoint = ({
...
}: {message: CheckpointEntry;threadId: string;messageIdx: number;isCheckpointGhost: boolean;threadIsRunning: boolean;}) => {
  const accessor = useAccessor();
  ...
  return (
    <div className={`void-flex void-items-center void-justify-center void-px-2 `}>
			<div
  ...
  if (role === "user") {
    ...
    return (
      <>
        ...
        <div
          className={`${
          isCheckpointGhost ? "void-opacity-50 void-pointer-events-none" : ""}`}>
...
const CommandBarInChat = () => {
  ...
  numFilesChanged === 1 ? "" : "s"} with changes`;
  ...
  <div className="void-void-scrollbar void-px-2 void-gap-1 void-w-full void-overflow-y-auto">
      ...
      <div className="void-flex void-px-4">
						<span className="void-text-void-fg-3 void-opacity-80">
  ...
  <button
    className={`void-flex void-items-center void-gap-1 void-rounded ${
    numFilesChanged === 0 ? "void-cursor-pointer" : "void-cursor-pointer hover:void-brightness-125 void-transition-all void-duration-200"}`}
  ...
  return (
    <>
        ...
        className={` void-select-none void-flex void-w-full void-rounded-t-lg void-bg-void-bg-3 void-text-void-fg-3 void-text-xs void-text-nowrap void-border-t void-border-l void-border-r void-border-zinc-300/10 void-px-2 void-py-1 void-justify-between `}>
				...
				<div className="void-flex void-gap-2 void-items-center">{fileDetailsButton}</div>
...
const EditToolSoFar = ({
  ...
  const desc1 =
  <span className="void-flex void-items-center">
			{uriDone ?
...
export const SidebarChat = () => {
  ...
  <ScrollToBottomContainer
    ...
    className={` void-flex void-flex-col void-px-4 void-py-4 void-space-y-4 void-w-full void-flex-1 void-min-h-0 void-overflow-x-hidden void-overflow-y-auto void-void-scrollbar ${
    ...
    <div className="void-px-2 void-my-1">
        ...
        showDismiss={true} />
					...
					<WarningBox
        className="void-text-sm void-my-2 void-mx-4"
        onClick={() => {
  ...
  <VoidChatArea
    ...
    onClickAnywhere={() => {
      ...
      keybindingString ? `${keybindingString} to add a selection. ` : ""}Enter instructions...`
      }
  ...
  <div className="void-flex void-flex-col void-gap-2 void-w-full void-text-nowrap void-text-void-fg-3 void-select-none">
			{[
  ...
  <div
    ...
    <div className="void-px-4 void-py-1">
        ...
        onSummarizeClick={handleSummarizeClick} />
			...
			<div className="void-px-2 void-pb-2">{inputChatArea}</div>
		</div>;


  const landingPageInput =
  <div>
			<div className="void-pt-8">{inputChatArea}</div>
		</div>;
  ...
  <div
    ...
    <ErrorBoundary>
					<div className="void-pt-8 void-mb-2 void-text-void-fg-3 void-text-root void-select-none void-pointer-events-none">
						Previous Threads
					</div>
					<PastThreadsList />
				</ErrorBoundary> :
