...
export const RuleBuilder: React.FC<RuleBuilderProps> = ({
	...
	const [namingPattern, setNamingPattern] = useState('{ProjectName}_{FileType}_{Version}');
	...
	const handlePatternChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
		...
		const updatedRules = rules.map(rule =>
			rule.type === 'rename'
				? { ...rule, pattern: newPattern, action: { ...rule.action, nameFormat: newPattern } }
				: rule
		);
	...
	const getPreviewName = useCallback((file: any) => {
			...
			<div>
				...
				<div style={{
					...
					<input
						...
						placeholder="e.g. {ProjectName}_{FileType}_{Version}"
					/>
					<div style={{
						marginTop: '8px',
						fontSize: '11px',
						...
						Available placeholders: &#123;ProjectName&#125;, &#123;FileType&#125;, &#123;Version&#125;, &#123;Date&#125;, &#123;Name&#125;
					</div>
				</div>
			</div>
						...
						<table style={{
							...
							<thead>
								<tr style={{
									...
									<th style={{ padding: '8px', textAlign: 'left', fontWeight: 600 }}>Original</th>
			...
			<div>
				...
				<div style={{
					...
					<div style={{
						...
						{rules.filter(r => r.type === 'tag').length} tagging rule(s) configured
					</div>
					<div style={{
						marginTop: '12px',
						fontSize: '11px',
