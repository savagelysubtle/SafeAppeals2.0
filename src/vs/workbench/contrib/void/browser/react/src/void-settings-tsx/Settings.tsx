...
const AddButton = ({
	...
	return (
		<button
			disabled={disabled}
			className={`bg-void-button-primary px-3 py-1 text-void-button-primary-text rounded-sm ${
				!disabled
					? "hover:bg-void-button-primary-hover cursor-pointer"
			...
			>
				...
				<textarea
					...
					className={`w-full min-h-[200px] p-2 rounded-sm border border-void-border-2 bg-void-bg-2 resize-none font-mono text-sm ${
						!overrideEnabled ? "text-void-fg-3" : ""
					}`}
					defaultValue={
			...
			{filteredModelDump.map((m, i) => {
				...
				const detailAboutModel =
					type === "autodetected" ? (
						<Asterisk
							size={14}
							className="inline-block align-text-top brightness-115 stroke-[2] text-void-button-primary"
							...
							{disabled ? null : (
								<div className="w-5 flex items-center justify-center">
									<button
										...
										className={`${
											hasOverrides ? "" : "opacity-0 group-hover:opacity-100"
										} transition-opacity`}
									>
			...
			{showCheckmark ? (
				<div className="mt-4">
					<AnimatedCheckmarkButton
						text="Added"
						className="bg-void-button-primary text-void-button-primary-text px-3 py-1 rounded-sm"
					/>
				</div>
			) : isAddModelOpen ? (
				<div className="mt-4">
