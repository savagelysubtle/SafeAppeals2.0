...
import { TerminalResolveReason } from '../../common/tools/toolsServiceTypes.js';
...
export const persistentTerminalNameOfId = (id: string) => {
	if (id === '1') return 'SafeAppeals Agent'
	return `SafeAppeals Agent (${id})`
}
export const idOfPersistentTerminalName = (name: string) => {
	if (name === 'SafeAppeals Agent') return '1'

	const match = name.match(/SafeAppeals Agent \((\d+)\)/)
