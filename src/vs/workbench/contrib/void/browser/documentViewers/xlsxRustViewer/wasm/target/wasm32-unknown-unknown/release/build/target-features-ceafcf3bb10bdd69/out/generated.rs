...
pub const CURRENT_TARGET: Target = {
    let arch = Architecture::Wasm;
    let target = Target::new(arch);
    let target = if let Ok(feature) = Feature::new(arch, "bulk-memory") { target.with_feature(feature) } else { target };
    let target = if let Ok(feature) = Feature::new(arch, "multivalue") { target.with_feature(feature) } else { target };
    let target = if let Ok(feature) = Feature::new(arch, "mutable-globals") { target.with_feature(feature) } else { target };
    let target = if let Ok(feature) = Feature::new(arch, "nontrapping-fptoint") { target.with_feature(feature) } else { target };
    let target = if let Ok(feature) = Feature::new(arch, "reference-types") { target.with_feature(feature) } else { target };
    let target = if let Ok(feature) = Feature::new(arch, "sign-ext") { target.with_feature(feature) } else { target };
    target
};
