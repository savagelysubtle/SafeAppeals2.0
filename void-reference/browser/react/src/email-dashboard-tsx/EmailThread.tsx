...
export const EmailThread: React.FC<EmailThreadProps> = ({
	...
	return (
		<div style={threadContainerStyle} className="overflow-hidden">
			{/* Thread Header - Clickable to expand/collapse */}
			<div
				onClick={handleToggleCollapse}
				className="cursor-pointer transition-all hover:bg-opacity-90"
			...
			>
				<div className="p-4 flex items-center gap-4">
					...
					<div className="flex-1 min-w-0">
						<div className="flex items-center gap-2 mb-1">
							...
							<div className="relative">
								...
								>
									<i className={`codicon ${statusBadge.icon}`} style={{ fontSize: '11px' }} />
									{statusBadge.label}
								</button>
								{/* Status dropdown menu */}
								{showStatusMenu && (
									<div
										className="absolute top-full left-0 mt-1 rounded shadow-lg z-50 overflow-hidden"
										style={{
									...
									>
										{(['needs-reply', 'awaiting-response', 'resolved', 'active'] as const).map((status) => {
											...
											return (
												...
												>
													<i className={`codicon ${badge.icon}`} style={{ color: badge.color, fontSize: '14px' }} />
													<span>{badge.label}</span>
													{thread.status === status && (
														<i className="codicon codicon-check ml-auto" style={{ fontSize: '12px' }} />
													)}
												</button>
											);
										})}
							...
							<h3 className="font-semibold text-sm truncate" style={textPrimaryStyle}>
								{thread.subject}
							</h3>
							{/* Email Count Badge */}
							<span
						...
						<div className="flex items-center gap-3 text-xs" style={textSecondaryStyle}>
							<span className="flex items-center gap-1">
								<i className="codicon codicon-person" style={{ fontSize: '12px' }} />
							...
							<span className="flex items-center gap-1">
								<i className="codicon codicon-clock" style={{ fontSize: '12px' }} />
					...
					<span className="text-xs flex-shrink-0" style={textSecondaryStyle}>
