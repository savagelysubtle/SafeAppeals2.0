...
export const ReviewChanges: React.FC<ReviewChangesProps> = ({
	...
	const fileOrganizerService = useMemo(() => {
		try {
			...
			console.error(
				"[ReviewChanges] Failed to get FileOrganizerService:",
				error
			);
			return null;
		}
	...
	const generatePreview = useCallback(async () => {
		...
		try {
			const previewChanges = await fileOrganizerService.previewChanges(
				files,
				rules
			);

			// Apply manual renames
			const changesWithRenames = previewChanges.map((change: any) => {
				const manualName = manualRenames[change.original.uri.toString()];
				if (manualName) {
					return {
						...change,
						proposed: {
			...
			setChanges(changesWithRenames);
			onChangesGenerated(changesWithRenames);
		} catch (error) {
			console.error("Failed to generate preview:", error);
		}
	}, [fileOrganizerService, files, rules, manualRenames, onChangesGenerated]);
	...
	const changesByFolder = useMemo(() => {
		...
		changes.forEach((change) => {
			const folderName =
				change.proposed.location?.path?.split("/").pop() || "Current Location";
			...
			{results.length > 0 && (
				...
				>
					<div style={{ fontWeight: 600, marginBottom: "8px" }}>
						{results.filter((r) => r.success).length} / {results.length} files
						processed successfully
					</div>
					{results.some((r) => !r.success) && (
						<div style={{ fontSize: "12px" }}>
							{results.filter((r) => !r.success).length} file(s) failed to
							process
						</div>
					)}
				</div>
