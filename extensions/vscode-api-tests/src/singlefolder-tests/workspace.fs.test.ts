...
suite('vscode API - workspace-fs', () => {
	...
	test('fs.write/stat/read/delete', async function () {
		...
		await vscode.workspace.fs.writeFile(uri, new Uint8Array(Buffer.from('HELLO')));
	...
	test('fs.delete folder', async function () {
		...
		await vscode.workspace.fs.writeFile(file, new Uint8Array(Buffer.from('FOO')));
	...
	test('fs.createFolder creates recursively', async function () {
		...
		await vscode.workspace.fs.writeFile(file, new Uint8Array(Buffer.from('Hello World')));
