...
suite('vscode API - workspace', () => {
	...
	test('openTextDocument, actual casing first', async function () {
		...
		fs.writeFile(uriOne, new Uint8Array(Buffer.from('one')), { create: true, overwrite: true });
		fs.writeFile(uriTwo, new Uint8Array(Buffer.from('two')), { create: true, overwrite: true });
	...
	test('Support creating binary files in a WorkspaceEdit', async function (): Promise<any> {
		...
		const data = new Uint8Array(Buffer.from('Hello Binary Files'));
	...
	test('encoding: decode', async function () {
		...
		// without setting
		assert.strictEqual(await vscode.workspace.decode(new Uint8Array(Buffer.from('Hello World')), uri), 'Hello World');
		assert.strictEqual(await vscode.workspace.decode(new Uint8Array(Buffer.from('Hellö Wörld')), uri), 'Hellö Wörld');
		...
		assert.strictEqual(await vscode.workspace.decode(new Uint8Array(Buffer.from('Hello World')), uri, { encoding: 'foobar123' }), 'Hello World');
