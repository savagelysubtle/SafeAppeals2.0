...
/**
 * Process-local coordinator for exclusive heavy ML slots
 * (whisper XOR diarization XOR embedding XOR docparse) plus a separate ffmpeg utility lane.
 * Serializes load/unload, queues FIFO waiters, cancels, idle-evicts, and enforces an
 * estimated peak RSS budget. Owned by safeappeals-ml; audio/RAG register adapters as consumers.
 */
export class MlResourceEngine implements vscode.Disposable {
	...
	constructor(options: Partial<MlEngineOptions> = {}, adapters: readonly ResourceAdapter[] = []) {
		...
		this.slots = {
			...
			docparse: { state: 'cold', refCount: 0 },
