...
import { isHeavyKind } from '../engineTypes';
import type { ResourceAdapter, ResourceKind } from '../engineTypes';
...
suite('MlResourceEngine', () => {
	...
	test('docparse is a heavy kind (XOR with whisper/diarization/embedding)', () => {
		assert.strictEqual(isHeavyKind('docparse'), true);
		assert.strictEqual(isHeavyKind('ffmpeg'), false);
	});

	test('acquiring docparse unloads whisper first', async () => {
		const whisper = new FakeAdapter('whisper', 100);
		const docparse = new FakeAdapter('docparse', 100);
		const engine = new MlResourceEngine({ idleUnloadMs: 60_000 }, [whisper, docparse]);

		await engine.withLease('whisper', { jobId: 'w' }, async () => { /* warm */ });
		assert.strictEqual(whisper.loaded, true);

		await engine.withLease('docparse', { jobId: 'ocr' }, async () => {
			assert.strictEqual(whisper.loaded, false);
			assert.strictEqual(docparse.loaded, true);
			assert.strictEqual(engine.getSnapshot().heavyKindLoaded, 'docparse');
		});
		assert.ok(whisper.unloadCount >= 1);
		await engine.dispose();
	});

	test('docparse XOR embedding: acquiring embedding unloads docparse', async () => {
		const docparse = new FakeAdapter('docparse', 100);
		const embedding = new FakeAdapter('embedding', 100);
		const engine = new MlResourceEngine({ idleUnloadMs: 60_000 }, [docparse, embedding]);

		await engine.withLease('docparse', { jobId: 'ocr' }, async () => { /* warm */ });
		assert.strictEqual(docparse.loaded, true);

		await engine.withLease('embedding', { jobId: 'emb' }, async () => {
			assert.strictEqual(docparse.loaded, false);
			assert.strictEqual(embedding.loaded, true);
		});
		assert.ok(docparse.unloadCount >= 1);
		await engine.dispose();
	});

	test('rejectIfBusy while docparse holds heavy lane', async () => {
		const docparse = new FakeAdapter('docparse', 100);
		const whisper = new FakeAdapter('whisper', 100);
		const engine = new MlResourceEngine({ idleUnloadMs: 60_000 }, [docparse, whisper]);

		const lease = await engine.acquire('docparse', { jobId: 'ocr-hold' });
		await assert.rejects(
			engine.acquire('whisper', { jobId: 'w', rejectIfBusy: true }),
			(err: unknown) => err instanceof MlBusyError,
		);
		await lease.release();
		await engine.dispose();
