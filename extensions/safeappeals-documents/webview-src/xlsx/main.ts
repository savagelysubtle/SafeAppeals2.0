/*--------------------------------------------------------------------------------------
 *  XLSX Rust Viewer - Webview Entry Point (Safe Appeals / safeappeals-documents)
 *  Runs inside a VS Code webview (Electron iframe).
 *  - Bundled as IIFE via extension esbuild (webview-src/xlsx → media/xlsx)
 *  - WASM loaded via host-posted binary or fetch + wasm-bindgen init
 *  - Production path: main-thread WASM (worker.ts / wasm-loader.ts are unused scaffolding)
 *--------------------------------------------------------------------------------------*/
...
window.addEventListener('message', async (event) => {
	...
	switch (message.type) {
		...
		case 'saveXLSX':
		case 'saveRequest':
			// Host saveCustomDocument / backup: fresh-serialize handshake (DOCX house pattern).
