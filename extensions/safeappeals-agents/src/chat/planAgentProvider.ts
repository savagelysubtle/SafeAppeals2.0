...
/**
 * Provides the SafeAppeals Plan agent as an extension agent.
 * This replaces the globalStorage-based provider in safeappeals-authentication.
 */
export class PlanAgentProvider implements vscode.ChatCustomAgentProvider, vscode.Disposable {
	...
	async provideCustomAgents(
		...
		return [{ uri: fileUri, sessionTypes: ['local', 'remote'] }];
	}

	private async writeCacheFile(content: string): Promise<vscode.Uri> {
		const cacheDir = vscode.Uri.joinPath(
			this.context.globalStorageUri,
			PLAN_AGENT_CACHE_DIR,
		);
		const fileUri = vscode.Uri.joinPath(cacheDir, PLAN_AGENT_FILENAME);
