...
export const CORE_AGENT_TOOL_NAMES: readonly string[] = [
	...
	'handle_dialog',
	'run_playwright_code',
	// Page-scoped CDP escape hatch (CORE, not ENSURED). Registered only when
	// workbench.browser.enableChatTools + agent sharing is available.
	'browser_cdp',
...
/**
 * Integrated browser CORE tools stripped in Plan mode — Plan must not drive
 * portals, dump case pages, or run CDP/Playwright against shared tabs.
 */
export const PLAN_MODE_BROWSER_DENYLIST: readonly string[] = [
	'browserTool',
	'open_browser_page',
	'click_element',
	'screenshot_page',
...
const PLAN_MODE_BROWSER_DENYLIST_SET = new Set(PLAN_MODE_BROWSER_DENYLIST);
...
/**
 * Whether a resolved tool id is an edit/write tool denied in Plan mode.
 */
...
/**
 * Whether a resolved tool id is a browser CORE tool denied in Plan mode.
 */
export function isPlanBrowserDeniedTool(name: string): boolean {
	return PLAN_MODE_BROWSER_DENYLIST_SET.has(name);
}

/**
 * Whether a resolved tool id is denied in Plan mode (edits or browser suite).
 */
export function isPlanDeniedTool(name: string): boolean {
	return isPlanEditDeniedTool(name) || isPlanBrowserDeniedTool(name);
}
...
/**
 * Removes Plan-mode edit + browser denylist tools from a selection.
 */
export function stripPlanDeniedTools<T extends { name: string }>(tools: readonly T[]): T[] {
	return tools.filter(tool => !isPlanDeniedTool(tool.name));
}
 ...
 * - When `modeName` is Plan, strips {@link PLAN_MODE_EDIT_DENYLIST} and
 *   {@link PLAN_MODE_BROWSER_DENYLIST} after selection (defense-in-depth).
 */
export function selectAgentTools(options: {
	...
	if (isPlanModeName(modeName)) {
		return stripPlanDeniedTools(selected);
	}
	return selected;
}
