...
async function writeProfileRule(profile: UserProfile): Promise<void> {
	...
	// Settings store canonical slugs; the human-readable rule uses display labels.
	const forRule = {
		...profile,
		jurisdiction: jurisdictionLabel(profile.jurisdiction) || profile.jurisdiction,
	};
	await vscode.workspace.fs.writeFile(uri, Buffer.from(renderProfileRule(forRule), 'utf8'));
}
...
export async function pickJurisdiction(current: string, title: string): Promise<string | undefined> {
	...
	const normalizedCurrent = normalizeJurisdictionId(current);
	const items: Array<vscode.QuickPickItem & { jurisdictionId?: string }> = [
		...JURISDICTIONS.map(j => ({
			label: jurisdictionLabel(j),
			description: j,
			picked: j === normalizedCurrent,
			jurisdictionId: j,
		})),
		{ label: custom },
	];
	const picked = await vscode.window.showQuickPick(items, {
		title,
		placeHolder: jurisdictionLabel(current) || 'e.g. BC WCB',
		ignoreFocusOut: true,
	});
	if (!picked) {
		return undefined;
	...
	if (picked.label !== custom) {
		return picked.jurisdictionId ?? normalizeJurisdictionId(picked.label);
	}
	const typed = await vscode.window.showInputBox({
		title,
		prompt: 'Compensation board / tribunal',
		value: jurisdictionLabel(current) || current,
		ignoreFocusOut: true,
	});
	return typed === undefined ? undefined : normalizeJurisdictionId(typed) || typed.trim();
}
