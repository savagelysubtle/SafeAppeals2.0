"""PowerPoint to PDF conversion using LibreOffice headless mode.

This module provides professional-quality PPTX to PDF conversion using LibreOffice's
native rendering engine. This approach produces output that matches Microsoft PowerPoint's
PDF export quality (95%+ formatting preservation).

Advantages over reportlab:
- Superior formatting preservation (slides, images, fonts, animations, layouts)
- Native PPTX engine (same as LibreOffice Impress)
- Preserves slide transitions and animations (as static slides)
...
logger = log_manager.get_converter_logger("pptx2pdf_libreoffice")


@converter(
    source_format="pptx",
    target_format="pdf",
    name="pptx_to_pdf_libreoffice",
    description="Convert PowerPoint to PDF using LibreOffice (professional quality)",
...
def convert_pptx_to_pdf_libreoffice(
    ...
    export_bookmarks: bool = True,  # Export slide titles as PDF bookmarks
    export_notes: bool = False,  # Don't export speaker notes
    # PDF/A Compliance
    ...
    **options: Any,
    ...
    """Convert PPTX to PDF using LibreOffice headless mode with enhanced quality.

    This converter produces Microsoft PowerPoint-quality PDF output with excellent
    ...
    Args:
        ...
        export_bookmarks: Export slide titles as PDF bookmarks. Default is True.
        export_notes: Export speaker notes. Default is False.
    ...
    Example:
        >>> # Basic conversion
        >>> convert_pptx_to_pdf_libreoffice("presentation.pptx")

        >>> # High quality for professional presentations
        >>> convert_pptx_to_pdf_libreoffice(
        ...     "presentation.pptx",
        ...     output_path="presentation_premium.pdf",
        ...     image_quality=95,
    ...
    publish(
        ConversionEvent(
            ...
            conversion_type="pptx2pdf",
            plugin_name="pptx_to_pdf_libreoffice",
            input_file=str(input_path),
            output_file=str(output_path),
        )
    )

    logger.info(f"Starting LibreOffice PPTX to PDF conversion: {input_path}")
    logger.info(f"Output will be saved to: {output_path}")

    start_time = time.time()

    try:
        # License validation and feature gating (pptx2pdf is paid-only)
        ...
        if not input_path.suffix.lower() in [".pptx", ".ppt", ".odp"]:
            raise ValueError(
                f"Input file must be .pptx, .ppt, or .odp, got: {input_path.suffix}"
            )
        ...
        # Get the appropriate filter name for PowerPoint
        filter_name = get_libreoffice_pdf_filter_name("pptx")
        ...
        record_conversion_attempt(
            converter_name="pptx2pdf_libreoffice",
            input_file=str(input_path),
            output_file=str(output_path),
            success=True,
        )
...
pptx_to_pdf_libreoffice = convert_pptx_to_pdf_libreoffice
